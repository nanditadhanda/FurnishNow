...
const { XRWebGLLayer } = window
  ...
  async function checkSupportedState() {
    await navigator.xr.isSessionSupported("immersive-ar").then((supported) => {
      ...
      if (support) {
        setXRButton("Try AR")
        document.getElementById('xr-button').addEventListener("click", onButtonClicked);
      } else {
        setXRButton( "AR not found")
      }
      document.getElementById('xr-button').disabled = !support;
    });
  }
  ...
  function onSessionStarted(session) {
    ...
    session.requestReferenceSpace("viewer").then((refSpace) => {
      session
        .requestHitTestSource({ space: refSpace }).then((hitTestSource) => {
  ...
  return (
    <div id="overlay">
      <div className="info-area">
        ...
        <button id="xr-button" disabled={!support}>{xrButton}</button>
      </div>
    </div>
  )
}
